...
def test_against_regex():
    ...
    for cp, cpd in dcp.items():
        for prop in cpd.Properties:
            props.update((prop,))
    for prop in props:
        if sys.maxunicode == 0xFFFF:
            assert all(regex.match(r'\p{{{0}}}$'.format(prop), s) for s in (chr(cp) for cp, cpd in dcp.items() if prop in cpd.Properties and cp <= 0xFFFF))
        else:
            assert all(regex.match(r'\p{{{0}}}$'.format(prop), s) for s in (chr(cp) for cp, cpd in dcp.items() if prop in cpd.Properties))
    ...
    for prop in props:
        if sys.maxunicode == 0xFFFF:
            assert all(regex.match(r'\p{{{0}}}$'.format(prop), s) for s in (chr(cp) for cp, cpd in pl.items() if prop in cpd.Properties and cp <= 0xFFFF))
        else:
            assert all(regex.match(r'\p{{{0}}}$'.format(prop), s) for s in (chr(cp) for cp, cpd in pl.items() if prop in cpd.Properties))
...
def test_confusables():
    ...
    assert cfs[0x1142F].confusable == (0x11434, 0x11442, 0x1142E)
